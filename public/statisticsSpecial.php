...
foreach ($estates as $estate) {
    $query = $db->query("SELECT inv_userid FROM inventory WHERE inv_itemid = {$estate}");
    while ($row = mysqli_fetch_assoc($query)) {
...
$query = $db->query("SELECT inv_userid FROM inventory WHERE inv_itemid = 625");
while ($row = mysqli_fetch_assoc($query)) {
    print ' &nbsp;&middot;&nbsp; ' . mafioso($row['inv_userid']) . '<br>';
}

print '<br>In October it was ' . itemInfo(603) . '.<br>';
$query = $db->query("SELECT inv_userid FROM inventory WHERE inv_itemid = 603");
while ($row = mysqli_fetch_assoc($query)) {
    print ' &nbsp;&middot;&nbsp; ' . mafioso($row['inv_userid']) . '<br>';
}
...
$query = $db->query("SELECT inv_userid FROM inventory WHERE inv_itemid = 607");
while ($row = mysqli_fetch_assoc($query)) {
    print ' &nbsp;&middot;&nbsp; ' . mafioso($row['inv_userid']) . '<br>';
}

print '<br>The ' . itemInfo(602) . ' is gained by attacking those significantly weaker than yourself.<br>';
$qdg = $db->query("SELECT inv_userid, sum(inv_qty) AS sumDagger FROM inventory WHERE inv_itemid = 602 GROUP BY inv_userid ORDER BY sumDagger DESC");
